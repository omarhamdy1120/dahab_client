...
if ( !$connection ) {
    ...
    if ( 'addusers' == $action ) {
        ...
        if ( $fname && $lname && $lname && $phone && $password ) {
            ...
            $query = "INSERT INTO users(fname,lname,email,phone,password) VALUES ('{$fname}','$lname','$email','$phone','$hashPassword')";
            mysqli_query( $connection, $query );
            header( "location:inc/users.php?id=allusers" );
        }
    } elseif ( 'updateusers' == $action ) {
        ...
        if ( $fname && $lname && $lname && $phone ) {
            $query = "UPDATE users SET fname='{$fname}', lname='{$lname}', email='$email', phone='$phone' WHERE id='{$id}'";
            mysqli_query( $connection, $query );
            header( "location:inc/users.php?id=allusers" );
        }
